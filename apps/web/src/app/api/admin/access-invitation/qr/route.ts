...
export async function GET(request: NextRequest) {
  ...
  // Only ever encode an invitation link back into this instance's access page.
  let target: URL;
  try {
    target = new URL(raw);
  } catch {
  ...
  if (target.origin !== new URL(request.url).origin || target.pathname !== '/access' || target.searchParams.get('mode') !== 'invite') {
    return apiError('Invalid url', 400);
  }
