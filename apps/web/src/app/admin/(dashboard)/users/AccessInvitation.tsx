...
/**
 * Issues an access invitation and renders it as a QR code, so someone can be
...
export function AccessInvitation() {
  ...
  const mint = async () => {
    ...
    setError('');
    try {
      const res = await fetch('/api/admin/access-invitation', { method: 'POST' });
      ...
      const qrRes = await fetch(`/api/admin/access-invitation/qr?url=${encodeURIComponent(body.data.url)}`);
