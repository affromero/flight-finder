...
import { AccessService, HouseholdProfileService, PrincipalManagement } from 'thesidedoor-core/access';
...
describe.skipIf(!databaseUrl)('shared access with PostgreSQL', () => {
  ...
  async function claimAdmin(): Promise<string> {
    const token = await service.claimOwner(await service.issueOperatorToken(), 'owner', 'owner account password', 'household');
    await new HouseholdProfileService(service).select(token, (await store.read()).principals[0]!.id);
    return token;
  }
  ...
  it('preserves deleted members hotel and car history while removing their notification destinations', async () => {
    ...
    const member = await store.managePrincipal(owner, await management.prepare({ kind: 'create', name: 'member' }));
  ...
  it('validates account creation and preserves household guest support without passwordless owners', async () => {
    ...
    cookieBoundary.token = await claimAdmin();
  ...
  it('rejects unsupported updates, self-removal and missing targets without changing accounts', async () => {
    ...
    cookieBoundary.token = await claimAdmin();
    await prisma.extractionConfig.update({ where: { id: 'singleton' }, data: { multiUserMode: true } });
    const ownerId = (await service.authenticate(cookieBoundary.token, true)).principal!.id;
    ...
    expect((await PATCH(request('PATCH', { isAdmin: false }), params)).status).toBe(400);
    ...
    expect(await prisma.user.count()).toBe(1);
    expect((await service.authenticate(cookieBoundary.token, true)).principal?.role).toBe('owner');
  });

  it('commits profile metadata and rejects per-profile passwords through the admin routes', async () => {
    vi.stubEnv('SELF_HOSTED', 'true');
    cookieBoundary.token = await claimAdmin();
    await prisma.extractionConfig.update({ where: { id: 'singleton' }, data: { multiUserMode: true } });
    const { POST } = await import('@/app/api/admin/users/route');
    ...
    const created = await POST(request('POST', { username: 'member', displayName: 'Member display' }));
    ...
    const memberSession = await service.enterHousehold('owner account password');
    await new HouseholdProfileService(service).select(memberSession, user.id);
    expect((await PATCH(request('PATCH', { displayName: 'Updated display' }), { params: Promise.resolve({ id: user.id }) })).status).toBe(200);
    expect((await service.authenticate(memberSession)).principal).toBeNull();
    expect((await PATCH(request('PATCH', { password: 'replacement account password', isAdmin: true }), { params: Promise.resolve({ id: user.id }) })).status).toBe(400);
    expect((await prisma.user.findUniqueOrThrow({ where: { id: user.id } })).displayName).toBe('Updated display');
  });

  it('keeps the first Admin and rejects a second Admin profile', async () => {
    const owner = await claimAdmin();
    const management = new PrincipalManagement(service);
    const firstId = (await service.authenticate(owner, true)).principal!.id;
    await expect(store.managePrincipal(owner, await management.prepare({ kind: 'create', name: 'second', password: 'second owner password', role: 'owner' }))).rejects.toMatchObject({ code: 'forbidden' });
    await expect(store.managePrincipal(owner, await management.prepare({ kind: 'delete', id: firstId }))).rejects.toMatchObject({ code: 'forbidden' });
  ...
  it('rejects a prepared account change after its authorizing owner session is revoked', async () => {
    const owner = await claimAdmin();
    const prepared = await new PrincipalManagement(service).prepare({ kind: 'create', name: 'member' });
  ...
  it('enables profile ownership and can disable the feature without losing Admin access', async () => {
    ...
    const { POST } = await import('@/app/api/admin/multi-user/route');
    const enabled = await POST(new NextRequest('http://localhost:3003/api/admin/multi-user', { method: 'POST', headers: { 'content-type': 'application/json' }, body: '{}' }));
    expect(enabled.status).toBe(201);
    const ownerId = (await service.authenticate(owner, true)).principal!.id;
    expect((await prisma.query.findUniqueOrThrow({ where: { id: tracker.id } })).userId).toBe(ownerId);
    ...
    const session = await service.enterHousehold('owner account password');
    await new HouseholdProfileService(service).select(session, (await store.read()).principals[0]!.id);
  ...
  it('rejects a member disabling profiles without changing either access mode', async () => {
    const owner = await claimAdmin();
    const created = await store.managePrincipal(owner, await new PrincipalManagement(service).prepare({ kind: 'create', name: 'member' }));
    await prisma.extractionConfig.update({ where: { id: 'singleton' }, data: { multiUserMode: true } });
    const member = await service.enterHousehold('owner account password');
    await new HouseholdProfileService(service).select(member, created!.id);
  ...
  it('reads admission policy without weakening managed gates or retaining deleted owners', async () => {
    ...
    const renewed = await service.enterHousehold('managed household password');
    await new HouseholdProfileService(service).select(renewed, ownerId);
    await service.setMode(renewed, 'individual');
  ...
  it('rolls back profile and access changes together when a profile constraint rejects the write', async () => {
    const owner = await claimAdmin();
    await store.managePrincipal(owner, await new PrincipalManagement(service).prepare({ kind: 'create', name: 'member' }));
