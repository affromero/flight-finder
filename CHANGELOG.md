...
Flight Finder now tracks hotels and car rentals alongside flights on your own instance. This release also adds hotel maps, account-based CLI model selection, and correct round-trip previews for longer stays.

### Added

- **Independent hotel tracking.** Search Google Hotels and Booking.com with dates, guests, room allocations, flexible stays, and policy filters. Follow the cheapest qualifying offer or a selected room/rate, inspect price history, and configure target-price alerts. The structured form works without AI. Hotel trackers have their own web and CLI controls and account-scoped history. ([#209](https://github.com/affromero/flight-finder/pull/209))
- **Independent car rental tracking.** Search DiscoverCars and Auto Europe with catalog-backed pickup and return locations, local dates and times, driver details, and rental preferences. An optional natural-language request produces a draft for review before searching. Track the cheapest qualifying offer or a particular contract, pause and resume checks, and inspect recorded prices and provider errors in the web app or CLI. ([#210](https://github.com/affromero/flight-finder/pull/210))
- **Rental price and contract evidence.** Results distinguish verified rental totals from advertised prices and estimates. Payment timing, deposits, excess, fuel, mileage, and driver restrictions retain their supporting terms. Unverified charges cannot qualify for price alerts. Provider errors and search limits remain visible. ([#210](https://github.com/affromero/flight-finder/pull/210))
- **Rental extras and protection review.** Inspect available protection products and request a fresh quote for the selected base rental. Supported DiscoverCars flows preserve selected seats and additional drivers through separate extras steps. Supplier estimates and unsupported Auto Europe extras are identified explicitly and remain ineligible for tracking as verified combined totals. ([#210](https://github.com/affromero/flight-finder/pull/210))
- **Recoverable rental actions.** Searches, tracker creation, manual refreshes, preferences, and protection requests retain request identities so an interrupted response can be recovered without duplicating the action. Account settings support personal provider preferences, and the CLI includes browsing, location selection, receipts, and recovery commands. ([#210](https://github.com/affromero/flight-finder/pull/210))
- **Hotel results maps.** Compare verified property locations and complete-stay prices, select rooms from a pin, and choose between hotels sharing a location. Maps load when opened and support personal styles, disabling maps, and administrator-configured providers. Missing coordinates or unavailable maps leave the offer list usable. ([#211](https://github.com/affromero/flight-finder/pull/211))
- **CLI model and thinking controls.** Setup, Settings, and Admin discover Codex models from the account signed in on the server. Select a model and supported thinking effort, retain the CLI's existing configuration, recheck authentication, and test a selection before saving. Saved unavailable models stay visible. ([#214](https://github.com/affromero/flight-finder/pull/214))
- **Managed CLI updates.** Administrators can update supported Docker installations from Settings or Admin. Updates verify the downloaded executable before switching versions and preserve credentials and saved settings. Unmanaged installations show an administrator command. ([#214](https://github.com/affromero/flight-finder/pull/214))
- **Optional private-instance access gate.** Protect instance pages with a shared password or administrator-issued invite links and QR codes. Password rotation revokes gate access independently of encrypted provider credentials. The CLI supports a machine access token. ([`1ccfb17`](https://github.com/affromero/flight-finder/commit/1ccfb17))

### Fixed

- **Flight CLI commands and source loading.** Root flight options reach their handlers alongside hotel and car subcommands. JSON errors and failed backend saves return failure status. Source commands load the generated Prisma client correctly, and browser links preserve literal tracker IDs. ([`a55d35d`](https://github.com/affromero/flight-finder/commit/a55d35d))
- **Portable CLI and tmux launchers.** Shared commands inherit caller configuration without requiring a particular secret manager or checkout path. New tmux panes retain that environment and the active CLI executable, preserve the selected model, close the initiating pane correctly, and surface launch failures. ([`a55d35d`](https://github.com/affromero/flight-finder/commit/a55d35d))
- **Chart dependency security.** Flight charts use Plotly's official basic bundle, removing its unused MapLibre dependency and the associated audit findings. Price history, booking links, VPN controls, and mobile rendering are covered by browser checks. The dependency tree has no reported npm audit vulnerabilities. ([`a55d35d`](https://github.com/affromero/flight-finder/commit/a55d35d))
- **Long-stay round-trip pricing.** Initial previews now try actual round-trip fares for every stay length. Previously, stays over 14 days added two one-way fares, even though later tracker updates could fetch the correct round-trip price. After a confirmed Google loading failure, separate-ticket estimates show both legs and booking links outside selectable fares. ([#218](https://github.com/affromero/flight-finder/pull/218), references [#216](https://github.com/affromero/flight-finder/issues/216), reported by @DcryptedCA)
- **Estimate isolation and preview retries.** Recognizable legacy one-way estimates are excluded from flight histories, alert baselines, community prices, and CLI summaries without deleting stored rows. Stale estimate submissions are rejected. Only actual fares are cached, search filters participate in cache identity, and a failed date pair no longer suppresses other stays on the route. ([#218](https://github.com/affromero/flight-finder/pull/218))
- **Reasoning-model query parsing.** Natural-language flight parsing handles complete reasoning blocks, fenced JSON, trailing objects, and nested wrappers without joining separate objects into invalid JSON. Existing normalization and clarification behavior are preserved. ([#208](https://github.com/affromero/flight-finder/pull/208), references [#201](https://github.com/affromero/flight-finder/issues/201), reported by @rachvin)
- **Hotel extraction and retention.** Google Hotel currency and whole-stay totals require visible evidence; consent and final redirects are checked before extraction. Standalone hotel searches retain their results correctly. ([#209](https://github.com/affromero/flight-finder/pull/209), [#210](https://github.com/affromero/flight-finder/pull/210))
- **Rental country validation.** Country choices use one canonical catalog, removing duplicates and rejecting unsupported codes consistently across inputs. ([#213](https://github.com/affromero/flight-finder/pull/213))
- **CLI authentication persistence.** Container restarts preserve rotated credentials, and CLI authentication failures include the available diagnostic output. Shared credential files retain the required group access. ([`a6ef445`](https://github.com/affromero/flight-finder/commit/a6ef445), [`6598856`](https://github.com/affromero/flight-finder/commit/6598856))
- **Root TypeScript script loading.** The root workspace explicitly declares `tsx`, so browser checks and local scripts continue to work when npm changes dependency placement. ([#217](https://github.com/affromero/flight-finder/pull/217))

### Changed

- **Public product and setup pages.** The landing page explains independent flight, hotel, and car trackers with visual installation steps. Private account and administrator views expose the corresponding travel controls. ([#209](https://github.com/affromero/flight-finder/pull/209), [#210](https://github.com/affromero/flight-finder/pull/210), [`1ddd26e`](https://github.com/affromero/flight-finder/commit/1ddd26e))
- **Shared travel execution.** Flight, hotel, car, and preview work coordinate admission and cancellation. Uncertain execution or VPN state is surfaced for administrator recovery, while notification delivery retains per-channel receipts. ([#210](https://github.com/affromero/flight-finder/pull/210))
- **Smaller runtime and bounded retention.** Runtime CLI dependencies are staged separately from development tooling. Startup cleanup bounds recognized CLI installations, and Compose declares log rotation limits. ([#215](https://github.com/affromero/flight-finder/pull/215))
- **Image and staging verification.** Builds record immutable source identity, staging checks use isolated resources, and Docker smoke tests avoid existing stacks. Container tooling uses a locked Prisma installation. ([#212](https://github.com/affromero/flight-finder/pull/212), [#210](https://github.com/affromero/flight-finder/pull/210))
- **Dependencies.** Updates include Anthropic SDK 0.123.0, OpenAI SDK 7.10.0, Next.js 16.3.4, next-intl 4.14.2, MapLibre 6.7.0, Nodemailer 9.1.1, Undici 8.10.1, Vitest and V8 coverage 5.0.0, tsx 4.23.13, type and lint tooling, and earlier npm and build-action updates since 0.14.0. ([#217](https://github.com/affromero/flight-finder/pull/217), [#206](https://github.com/affromero/flight-finder/pull/206), [#204](https://github.com/affromero/flight-finder/pull/204), [#200](https://github.com/affromero/flight-finder/pull/200))

### Upgrade notes

- Back up the database before upgrading from 0.14.0. This release includes additive hotel, car, shared-job, map-preference, and CLI configuration storage. The container startup applies the current schema and shared travel constraints; source installations should run `npm run db:push`. ([#209](https://github.com/affromero/flight-finder/pull/209), [#210](https://github.com/affromero/flight-finder/pull/210), [#211](https://github.com/affromero/flight-finder/pull/211), [#214](https://github.com/affromero/flight-finder/pull/214))
- Hotel and car searches run on self-hosted instances and remain independent of flight trackers. Bookings are completed with providers. Google Hotels supports one room; Booking.com supports multiple rooms. Unverified hotel and rental charges remain unknown. ([#209](https://github.com/affromero/flight-finder/pull/209), [#210](https://github.com/affromero/flight-finder/pull/210))
- Opening a hotel map sends the tile provider the browser's IP address and requested map area. Account details, stay dates, and prices are excluded from tile requests. Users can disable maps. ([#211](https://github.com/affromero/flight-finder/pull/211))
- For development checks with Vitest 5, use a supported Node release: Node 22.12 or later within 22, Node 24, or Node 26 and later. ([#217](https://github.com/affromero/flight-finder/pull/217))
- Supply local CLI and maintenance configuration through the caller environment. If you use a secret manager, wrap the command externally with your own configuration. Shared scripts no longer require an Infisical or Doppler account. ([`a55d35d`](https://github.com/affromero/flight-finder/commit/a55d35d))
