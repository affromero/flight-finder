...
If multiple people in your household want to track their own flights without
sharing a dashboard, enable multi user mode.

**How to enable:** during the setup wizard (last optional step) or later
from the admin Settings page. Toggling on asks for an admin username
and password and atomically creates the first admin user.

**What it changes:**

- A unified `/login` page replaces the password only admin form
- The landing page redirects to `/login` until you sign in
- Each user gets `/account` with only their own trackers, and
  `/account/settings` for currency, country, airlines, and cabin class
  defaults
- The admin gains an `/admin/users` page to add, reset passwords,
  promote, demote, or delete users; per query reassignment lives on
  the existing `/admin/queries` table
- Anonymous `POST /api/queries` returns 401 in this mode so no orphan
  trackers land on the dashboard

**Backfill on enable:** every existing unowned non seed tracker is
assigned to the new admin user. A one time dismissible banner appears
on `/admin/users` so you can reassign any that belong to other
household members.

**Hosted (fairtrail.org)** does NOT offer multi user mode and never
will — the public site is read only / single tenant by design.
</details>
