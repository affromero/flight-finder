...
Account recovery (self hosted multi user mode):
  ...
  access recover <principalId>           Create a 15-minute recovery code
...
Sidedoor owns household admission, the shared password, passkeys,
sessions, invitations, recovery codes, and API device tokens. Flight Finder's
`User` rows contain product preferences only.

The installer prints a 15-minute Admin claim code. Open `/access` and claim
the first profile. Its password becomes the one shared app password. After
...
on later visits. Admin can revoke household passkeys at `/access/security`.

Anyone admitted to the household can select the first Admin profile and manage
...
For local recovery, list principals and create a short-lived recovery code:

```bash
flight-finder access list
flight-finder access recover <principalId>
```

Enter the code at `/access?mode=recover`, set a new password, and enroll a new
passkey. Recovery increments the principal epoch and revokes existing sessions
...
a fresh install it also prints a 15-minute owner claim code. Enter that code at
`/access`. Repeating initialization preserves stored passwords, passkeys,
