...
| **Anthropic** | Saved encrypted in setup or Settings | Pay-per-token | Claude Haiku 4.5 (default) |
| **OpenAI** | Saved encrypted in setup or Settings | Pay-per-token | GPT-4.1 Mini |
| **Google** | Saved encrypted in setup or Settings | Pay-per-token | Gemini 2.5 Flash |
...
| **OpenAI + custom URL** | Endpoint and credential saved in Settings | Varies | OpenRouter or any OpenAI-compatible endpoint |

**Three ways to use Flight Finder:**

- **Subscription users** (Claude Pro/Max, ChatGPT Pro) -- auto-detected, auth tokens mounted read-only.
- **API key users**: paste a key in setup or Settings. Sidedoor stores it encrypted.
- **Local model users**: select Ollama, llama.cpp, or vLLM in the admin UI and save the endpoint and model.
...
Shared access (self hosted):
  access setup                           Create the first Admin and shared password
  access reset                           Replace the shared password locally
  access list                            List Sidedoor principals
  access device <principalId> [name]     Create a scoped API device token
```
...
`FLIGHT_FINDER_SESSION` in the process environment. Private instances also
accept a Sidedoor device token through `FLIGHT_FINDER_TOKEN`. Create one locally
with `flight-finder access device <principalId> [name]`. Credentials go only to
the selected server; redirects are rejected. Hotel commands never change the flight
...
Deployment settings are in `~/.flight-finder/.env` (generated by the installer). AI settings live in Sidedoor and are managed in the browser:
...
<details>
<summary>One password, separate profiles</summary>

Sidedoor owns app admission, the shared password, passkeys,
sessions, and API device tokens. Flight Finder's
`User` rows contain product preferences only.

Run `flight-finder access setup` locally to create the first Admin profile and
shared password before opening the app. After
entering it, a visitor can save a passkey with Apple Passwords or another
WebAuthn manager before choosing a profile. A saved passkey opens the picker
on later visits. The entry screen shows only Password and Continue. Admin can
revoke passkeys in App access at `/access/security`.

Anyone admitted to the app can select the first Admin profile and manage
settings. Selecting another profile removes Admin access. Admin manages member
profiles at `/admin/users`. Members have separate trackers and preferences and
need no second password. `/account` and `/account/settings` use the selected
profile.

If the shared password is lost, reset it locally on the server:

```bash
flight-finder access reset
```

The command asks for a new password without echoing it. Reset revokes existing
sessions and passkeys. Enroll a new passkey after entering the app.
...
Owner access is required at `/admin`:
...
npm run cli -- access setup
npm run dev
```

### Shared backend initialization

Flight Finder uses Sidedoor for access and provider credentials. Initialize it
before the first start, with every process that writes to the database stopped.

For a Compose installation, after stopping external writers:
...
Preparation applies the schema and creates Sidedoor access and provider state. On
a fresh install run `npm run cli -- access setup` locally before starting the web
app. Repeating initialization preserves stored passwords, passkeys,
sessions, provider credentials, and policy.
