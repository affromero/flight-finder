...
Sidedoor owns household admission, owner and member credentials, passkeys,
sessions, invitations, recovery codes, and API device tokens. Flight Finder's
`User` rows contain product preferences only.

The installer prints a 15-minute owner claim code. Open `/access`, claim the
owner, then configure household access and enroll a passkey at
`/access/security`. Sidedoor stores access policy in the application database.

Owners manage household members at `/admin/users`. Each member has separate
trackers and preferences. A member without a password or passkey can be selected
after household admission. Protected members authenticate with their own
password or passkey. `/account` and `/account/settings` use the selected or
